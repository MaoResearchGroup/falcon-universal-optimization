...
def main():
  ...
  run_mantis_formatter = False # legacy LNP-only formatter for MANTIS liquid handler input

  # Optional metadata columns to copy from the last non-empty dataset row when
  # exporting optimized suggestions. Leave empty for fully generic datasets.
  carry_forward_metadata_cols = []
  ...
  if run_optimization == True:
    ...
    formula_col = next((col for col in ['Formula_label', 'Formula_Label'] if col in df_existing.columns), None)
    ...
    metadata_values = {}
    for col in carry_forward_metadata_cols:
        if col not in df_existing.columns:
            raise KeyError(f"carry_forward_metadata_cols contains missing column: {col}")
        non_empty_values = df_existing[col].dropna()
        metadata_values[col] = non_empty_values.iloc[-1] if len(non_empty_values) else None
  ...
  if run_mantis_formatter == True:
    required_mantis_params = {'NP_ratio', 'IL+HL', 'HL_IL+HL', 'PEG_PEG+Chol'}
    if not required_mantis_params.issubset(input_param_names):
      raise ValueError(
        "The MANTIS formatter is legacy LNP-specific and requires "
        f"these input parameters: {sorted(required_mantis_params)}"
      )
