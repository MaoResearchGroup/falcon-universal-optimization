...
def get_learning_curve(pipeline, refined = False, NUM_ITER =5, num_splits =5, num_sizes= 50):
    ...
    train_size = np.linspace(1, max_train, min(num_sizes, max_train)).astype(int)
    train_size = np.unique(train_size)
    train_scores_mean = None
    validation_scores_mean = None
    ...
    for i in range(NUM_ITER):
        ...
        if train_scores_mean is None:
            train_scores_mean = pd.DataFrame(index=train_sizes)
            validation_scores_mean = pd.DataFrame(index=train_sizes)
